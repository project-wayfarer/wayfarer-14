...
namespace Content.Server.NPC.Pathfinding
{
    ...
    public sealed partial class PathfindingSystem : SharedPathfindingSystem
    {
        ...
        {
            var mapUidA = coordsA.GetMapUid(EntityManager); // Misfits
            var mapUidB = coordsB.GetMapUid(EntityManager); // Misfits
            handle = -1;
            ...
            var gridUidA = coordsA.GetGridUid(EntityManager); // Misfits
            var gridUidB = coordsB.GetGridUid(EntityManager); // Misfits
        ...
        public bool RemovePortal(int handle)
        {
            ...
            var gridUidA = portal.CoordinatesA.GetGridUid(EntityManager); // Misfits
            var gridUidB = portal.CoordinatesB.GetGridUid(EntityManager); // Misfits
        ...
        {
            if (!TryComp<TransformComponent>(entity, out var start)) // Misfits
        ...
        public async Task<PathResultEvent> GetPath(
            ...
            PathFlags flags = PathFlags.None)
        {
            if (!TryComp<TransformComponent>(entity, out var xform) || // Misfits
                !TryComp<TransformComponent>(target, out var targetXform)) // Misfits
                return new PathResultEvent(PathResult.NoPath, new List<PathPoly>());
        ...
        {
            var gridUid = coordinates.GetGridUid(EntityManager); // Misfits

            if (!TryComp<GridPathfindingComponent>(gridUid, out var comp) ||
                !TryComp<TransformComponent>(gridUid, out var xform)) // Misfits
            {
                return null;
            }

            var localPos = Vector2.Transform(coordinates.ToMapPos(EntityManager, _transform), xform.InvWorldMatrix); // Misfits
            ...
            var polys = chunk.Polygons[(int) chunkPos.X * ChunkSize + (int) chunkPos.Y]; // Misfits
